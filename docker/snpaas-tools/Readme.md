...
Subcommands:
  ...
  Bosh/deployment subcommands
    ...
    destroy [-f] [bosh-destroy-parameters]
                    Delete deployment (does not delete cloud/runtime configs) but 
                    deletes all credhub secrets.
...
* `terraform` contains terraform files. If present it needs to be initialized with
  `terraform init` and the script will execute `terraform output --json` and generate
  another file in `variables/terraform.yml` with the variables defined in the output.
...
* Easily manage Credhub secrets in a deployment. One can import, export and
  ...
  are managed via the 'variables' section in a manifest!). Also, deleting a
  deployment removes all credhub secrets.
